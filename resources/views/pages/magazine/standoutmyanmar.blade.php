	...
	<div class="col_one_third">
		<div class="ipost clearfix">
			...
			<div class="entry-title">
				<h3><a href="{{ url('/postdetailsmyanmar', $standout->id) }}">{{ $standout->mname }}</a></h3>
			</div>
			<ul class="entry-meta clearfix">
				<li><i class="icon-calendar3"></i>{{ $standout->created_at }}</li>
				<li><a href="{{ url('/postdetailsmyanmar', $standout->id) }}"><i class="icon-comments"></i>{{ $tcmt }}</a></li>
			</ul>
			<div class="entry-content">
				<p><?php echo substr($standout->mdescription,0, 200) ?>...</p>
			</div>
		</div>
	</div>
	@endforeach
